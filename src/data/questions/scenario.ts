...
**落地**：每个维度按 1~5 打分 × 权重求和得出综合分，同时记录“不可量化因素”（如团队意愿），最终给出推荐 + 备选方案 + 回退路径。`,
  ...
  },
  {
    ...
    diff: 'medium',
    sub: '请求竞态',
    q: '搜索框连续输入触发多个请求，后返回的旧请求覆盖新结果怎么办？',
    ans: `**面试回答：**这是典型请求竞态问题：用户输入 abc 时可能发出 a、ab、abc 三个请求，如果 a 请求最后返回，就可能覆盖 abc 的结果。

**解决方案**：
1. 输入侧防抖，减少请求数量；
2. 每次请求生成递增序号 requestId，只允许最新 requestId 的响应更新页面；
3. 新请求发出前用 AbortController 取消上一次未完成请求；
4. loading 状态和空态要绑定当前关键词，避免旧状态闪烁；
5. 缓存结果时 key 要包含完整查询参数，不能只按接口路径缓存。

**项目表达**：在复杂筛选列表中，分页、关键词、筛选条件变化都可能触发请求，最稳的是把查询参数收敛成一个稳定 queryKey，用它管理请求、缓存和响应落地。`,
    ana: '请求竞态的核心是“只让最后一次有效请求落地”，AbortController 和 requestId 是常用组合。',
    keys: ['请求竞态', '防抖', 'AbortController', 'requestId', 'queryKey'],
    src: '问答类型面试题.md',
  },
  {
    id: 'sc-021',
    ...
    diff: 'medium',
    sub: '表单体验',
    q: '复杂表单需要支持草稿自动保存时，下面哪个做法更合理？',
    opts: [
      '每次输入都立即同步提交正式数据，并跳过校验',
      '对表单变化做防抖保存草稿，区分草稿接口和正式提交接口',
      '只把草稿存在内存中，刷新页面直接丢失',
      '把所有用户草稿都写到同一个 localStorage key',
    ],
    ans: 'B',
    ana: '草稿自动保存应降低打扰且避免污染正式数据。常见做法是变化后防抖保存，草稿和正式提交分开；key 或接口参数包含用户、业务 ID、版本等信息；恢复时提示用户是否继续编辑旧草稿。',
    keys: ['草稿保存', '防抖', '正式提交分离', '恢复提示'],
    src: '问答类型面试题.md',
  },
  {
    id: 'sc-022',
    ...
    diff: 'medium',
    sub: '弱网与离线',
    q: '移动端或后台系统在弱网、断网场景下如何做体验兜底？',
    ans: `**面试回答：**弱网兜底要先区分核心链路和非核心链路：核心操作要明确成功、失败或可重试，非核心内容可以降级展示缓存或占位。

**具体措施**：
1. 通过 navigator.onLine、请求超时、错误码和心跳判断网络状态，但不要只依赖单一 API；
2. 请求设置超时、取消和有限重试，重试要有退避，避免网络恢复前持续轰炸；
3. 列表、详情可展示上一次成功缓存，并标记数据可能不是最新；
4. 表单提交失败时保留用户输入，支持稍后重试；
5. 上传、流式输出等长任务要支持暂停、续传、重连和进度恢复；
6. 页面层给出明确状态：加载中、超时、断网、重试按钮，而不是一直转圈。

重点是让用户知道当前状态，并尽量保住已经输入或已经完成的进度。`,
    ana: '弱网题的评价点是状态清晰、可重试、保留用户成果，而不是简单弹一个网络错误。',
    keys: ['弱网', '退避重试', '缓存兜底', '表单保留', '续传'],
    src: '问答类型面试题.md / 知识点快速复习指南.md',
  },
  {
    id: 'sc-023',
    ...
    diff: 'medium',
    sub: '接口治理',
    q: '前端做接口请求治理时，下面哪些能力是常见且有价值的？（多选）',
    opts: [
      '请求超时和取消',
      '重复请求去重或合并',
      '统一错误码处理和登录态失效处理',
      '把所有后端错误都静默吞掉，不给用户任何反馈',
    ],
    ans: ['A', 'B', 'C'],
    ana: '接口治理的目标是让请求行为稳定、可控、可观测。超时、取消、去重、合并、错误码处理、401 静默刷新或跳转登录都很常见。错误不能全部静默吞掉，关键链路应明确提示并保留重试入口。',
    keys: ['请求取消', '请求去重', '错误码处理', '401', '重试'],
    src: '问答类型面试题.md / 针对简历问答.md',
  },
  {
    id: 'sc-024',
    ...
    diff: 'medium',
    sub: '错误监控',
    q: '错误监控中为什么需要记录用户行为面包屑？一般记录哪些信息？',
    ans: `**面试回答：**错误堆栈只能说明代码在哪一行报错，但不一定能还原用户怎么走到这个状态。用户行为面包屑用于记录错误发生前的关键上下文，帮助复现和定位问题。

常见记录内容包括：路由跳转、点击行为、接口请求和响应状态、控制台错误、关键状态变化、性能指标、用户环境信息和版本号。上报时要注意采样、脱敏和隐私边界，不能把密码、Token、身份证号等敏感信息直接带上去。`,
    ana: '面包屑是错误监控从“知道报错”走向“能复现链路”的关键能力，也要强调脱敏。',
    keys: ['面包屑', '错误监控', '路由', '接口请求', '脱敏'],
    src: '问答类型面试题.md / 针对简历问答.md',
  },
]
