...
**补充**：defer 脚本会在 DOM 解析完成后、DOMContentLoaded 之前按顺序执行；async 脚本下载完立即执行，执行时机不确定。`,
  ...
  },
  {
    ...
    diff: 'medium',
    sub: '浏览器安全',
    q: '关于 Content Security Policy（CSP），下列哪些说法是正确的？（多选）',
    opts: [
      'CSP 可以限制脚本、样式、图片等资源的加载来源',
      'CSP 有助于降低 XSS 攻击造成的脚本执行风险',
      '配置 CSP 后就不需要做输出转义',
      'CSP 可以通过响应头 Content-Security-Policy 下发',
    ],
    ans: ['A', 'B', 'D'],
    ana: 'CSP 是浏览器安全策略，可以限制资源来源、禁止内联脚本或要求 nonce/hash，从而降低 XSS 后果。但 CSP 是纵深防御，不能替代输出转义、富文本白名单、HttpOnly Cookie 等基础防护。',
    keys: ['CSP', '资源来源限制', 'XSS 防御', '响应头'],
    src: '浏览器原理知识点.md / 收集的面试知识点.md',
  },
  {
    id: 'br-023',
    ...
    diff: 'medium',
    sub: 'Service Worker / PWA',
    q: 'Service Worker 可以直接操作页面 DOM，因此适合把页面渲染逻辑放到 Service Worker 中执行。',
    ans: false,
    ana: 'Service Worker 运行在独立线程中，不能直接访问 DOM。它更适合拦截网络请求、做缓存策略、离线能力、消息推送和后台同步。页面渲染仍应由主线程上的页面脚本负责。',
    keys: ['Service Worker', '不能操作 DOM', '离线缓存', '请求拦截'],
    src: '浏览器原理知识点.md / 从零开始的前端面试题.md',
  },
]
