...
如果是后台系统，优先使用系统字体栈，通常比引入大体积自定义字体更稳。`,
  ...
  },
  {
    ...
    diff: 'medium',
    sub: '监控排查',
    q: '线上性能数据统计中，为什么通常更关注 P75 而不是单次本地 Lighthouse 分数？',
    opts: [
      'P75 更能反映大多数真实用户在真实网络和设备下的体验',
      'P75 表示所有用户中最快的 25%',
      'Lighthouse 永远不能用于定位问题',
      '只要 P75 好，错误率就一定为 0',
    ],
    ans: 'A',
    ana: '本地 Lighthouse 适合定位问题，但受设备、网络和运行环境影响较大。P75 表示 75% 用户体验不差于该值，更适合作为真实用户监控口径，用于衡量线上整体体验。',
    keys: ['RUM', 'P75', 'Lighthouse', '真实用户体验'],
    src: '前端性能优化.md / 知识点快速复习指南.md',
  },
  {
    id: 'pf-019',
    type: 'multiple',
    diff: 'medium',
    sub: '资源优化',
    q: '关于前端资源压缩与传输优化，下列哪些说法是正确的？（多选）',
    opts: [
      '开启 gzip 或 brotli 可以减少文本资源传输体积',
      'JS/CSS 压缩可以去除空白、注释并做安全的代码简化',
      '图片压缩和格式转换可以降低首屏资源体积',
      '压缩资源后就不再需要缓存策略',
    ],
    ans: ['A', 'B', 'C'],
    ana: '压缩和缓存解决的是不同问题：压缩减少单次传输体积，缓存减少重复请求或响应体下载。文本资源适合 gzip/brotli，图片要结合 WebP/AVIF、尺寸裁剪和响应式图片处理。',
    keys: ['gzip', 'brotli', '资源压缩', '图片优化', '缓存策略'],
    src: '前端性能优化面试题.md / 前端性能优化.md',
  },
]
