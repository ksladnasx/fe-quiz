...
这样后端字段变化时，优先改接口封装和类型定义，页面组件只感知稳定的业务模型。`,
  ...
  },
  {
    ...
    diff: 'medium',
    sub: '项目表达',
    q: '面试中介绍项目难点时，下面哪种回答更能体现真实参与和技术深度？',
    opts: [
      '这个项目比较复杂，很多地方都很难',
      '我负责页面开发，后端和整体链路不太清楚',
      '先说明具体业务背景和问题，再讲自己的方案、取舍、踩坑和最终结果',
      '只列出项目使用了 React、Vue、FastAPI 等技术栈',
    ],
    ans: 'C',
    ana: '项目难点表达要具体：背景、目标、方案、难点、结果。只说复杂或只罗列技术栈都缺少可验证细节；能讲出取舍、失败点和结果，才更像真实参与。',
    keys: ['项目表达', '背景目标方案', '技术取舍', '结果口径'],
    src: '知识点快速复习指南.md / 针对简历问答.md',
  },
  {
    id: 'pj-028',
    ...
    diff: 'medium',
    sub: '团队协作',
    q: '当前后端接口还没完成但前端排期紧张时，你会如何推进联调和交付？',
    ans: `**面试回答：**我会先和后端确认接口契约，包括请求参数、返回结构、错误码、分页格式和空值语义，然后前端基于 Mock 或接口适配层先开发页面主流程，避免完全等待。

同时把 Mock 数据和真实接口差异收敛在 API 层，页面组件只依赖稳定的业务模型。接口逐步完成后，通过 Network、Swagger、日志和联调用例验证字段、类型、权限和异常状态。对于我能独立处理且风险可控的后端任务，可以在确认边界后协助补位；但涉及核心业务规则或高风险数据变更时，应和负责同事保持评审和确认。

核心是既保证进度，也不绕过协作边界和质量控制。`,
    ana: '这道题来自文档里的跨角色协作经历，重点是接口契约、Mock 收敛、联调验证和边界意识。',
    keys: ['接口契约', 'Mock', 'API 适配层', 'Swagger', '协作边界'],
    src: '针对简历问答.md / 收集的面试知识点.md / fastapi知识点.md',
  },
]
