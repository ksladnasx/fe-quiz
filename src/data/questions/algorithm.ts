...
**选择**：要求“最少几步、最短距离、按层遍历”优先 BFS；要求“枚举所有可能、判断是否存在路径、递归结构明显”优先 DFS。`,
  ...
  },
  {
    ...
    diff: 'medium',
    sub: '数组与字符串',
    q: '在有序数组中查找目标值时，二分查找的时间复杂度通常是多少？',
    opts: ['O(1)', 'O(log n)', 'O(n)', 'O(n log n)'],
    ans: 'B',
    ana: '二分查找每次比较后都能排除一半搜索区间，因此时间复杂度是 O(log n)。前提是数据有序，并且能通过下标随机访问。链表不适合普通二分，因为访问中点需要线性遍历。',
    keys: ['二分查找', '有序数组', 'O(log n)', '随机访问'],
    src: '基础算法.md / 知识点快速复习指南.md',
  },
  {
    id: 'al-026',
    ...
    diff: 'medium',
    sub: '链表',
    q: '如何判断链表是否有环？如果有环，如何找到入环节点？',
    ans: `**面试回答：**可以使用快慢指针。slow 每次走一步，fast 每次走两步；如果链表有环，两个指针最终会在环内相遇；如果 fast 走到 null，则说明没有环。

找到入环节点时，让一个指针从头节点出发，另一个指针从相遇点出发，二者每次都走一步，再次相遇的位置就是入环节点。

原因可以从距离关系推导：相遇时 fast 比 slow 多走的距离正好是环长度的整数倍，因此从头节点到入口的距离，等于相遇点继续走到入口的距离。`,
    ana: '链表环题的核心是快慢指针。能说出“相遇后一个从头走、一个从相遇点走”基本就完整了。',
    keys: ['快慢指针', '链表环', '入环节点', 'Floyd 判圈'],
    src: '基础算法.md / 知识点快速复习指南.md',
  },
]
