...
一句话：前端环境变量用于控制构建和运行入口，不是秘密存储。`,
  ...
  },
  {
    ...
    diff: 'medium',
    sub: '工程化综合',
    q: '前端项目中 sourcemap 的作用和使用注意点有哪些？（多选）',
    opts: [
      '帮助把线上压缩后的错误堆栈映射回源码位置',
      '可以辅助错误监控平台定位真实报错文件和行列号',
      '生产环境 sourcemap 应结合权限和访问控制，避免源码泄露',
      '开启 sourcemap 后就不需要错误监控和版本号',
    ],
    ans: ['A', 'B', 'C'],
    ana: 'sourcemap 能把构建产物映射回源码，对线上错误定位很有价值。但生产环境要控制访问权限，通常上传到监控平台或内网存储，而不是公开暴露。版本号仍然重要，用于将错误堆栈与对应构建产物关联。',
    keys: ['sourcemap', '错误定位', '源码泄露', '版本号'],
    src: '前端工程化面试题.md / 问答类型面试题.md',
  },
  {
    id: 'en-018',
    type: 'essay',
    diff: 'medium',
    sub: 'npm 与包管理',
    q: 'pnpm 相比 npm/yarn 有什么特点？为什么它能节省磁盘空间？',
    ans: `**面试回答：**pnpm 的核心特点是内容寻址存储和硬链接。依赖包会先存到全局 store，同一版本的包只保存一份，项目 node_modules 中通过硬链接引用，因此多个项目复用依赖时能明显节省磁盘空间并提升安装速度。

pnpm 还使用更严格的 node_modules 结构，项目不能随便访问没有声明的幽灵依赖，有助于暴露依赖声明不完整的问题。Monorepo 场景下，pnpm workspace 可以管理多个包的依赖和脚本，适合前端工程化项目。`,
    ana: '回答 pnpm 时重点说清全局 store、硬链接、严格依赖和 workspace，而不是只说“更快”。',
    keys: ['pnpm', '全局 store', '硬链接', '幽灵依赖', 'workspace'],
    src: '前端工程化面试题.md / 知识点快速复习指南.md',
  },
]
